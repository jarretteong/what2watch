...
import { fetchTMDBMovieCredits, fetchTMDBMovieImages, fetchTMDBMovieVideos, fetchTMDBRecommendedMovies } from '@/app/utils/tmdbApi';
import { Credits } from '@/interfaces/credits';
import { ImageData, MovieCustom, Video } from '@/interfaces/movie';
import _ from 'lodash';
import type { NextApiRequest, NextApiResponse } from 'next'

type RecommendedMovieData = {
    data: MovieCustom[];
    page: number;
...
type RecommendedMovieError = {
    error: string;
};

const handler = async (
  req: NextApiRequest,
  res: NextApiResponse<RecommendedMovieData | RecommendedMovieError>
) => {
    if (req.query.id) {
        const movieId =
        ...
        const movies = await fetchTMDBRecommendedMovies(+movieId!, page);
