...
import { fetchTMDBMovieCredits, fetchTMDBMovieImages, fetchTMDBMovieVideos, fetchTMDBRecommendedMovies, fetchTMDBSimilarMovies } from '@/app/utils/tmdbApi';
...
type SimilarMovieError = {
    error: string;
};

const handler = async (
  req: NextApiRequest,
  res: NextApiResponse<SimilarMovieData | SimilarMovieError>
) => {
    if (req.query.id) {
        const movieId =
        ...
        const movies = await fetchTMDBSimilarMovies(+movieId!, page);
