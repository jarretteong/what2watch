...
const MoviePoster: React.FunctionComponent<GenreProps> = ({
    ...
    const [selectedMovie, setSelectedMovie] = useState<MovieCustom>(
        _.first(movieList.filter(m => m.poster_path))!
    );
    ...
    useEffect(() => {
        if (movies.length > 0) {
            setSelectedMovie(_.first(movies)!);
        }
    }, [movies]);
    ...
    if (
        !isLoading &&
        !isError &&
        _.flatMap(data?.pages, "data").filter((m) => m.poster_path).length !==
            movies.filter((m) => m.poster_path).length
    ) {
        const updatedMovies = _.flatMap(data?.pages, "data").filter((m) => m.poster_path);
            ...
            return (
                <div className={styles.genrePosterContainer}>
                    ...
                    <ReactSwiper
                        ...
                        onClick={(swiper) => {
                            setSelectedMovie(
                                movies?.[swiper.clickedIndex] || _.first(movies)
                            );
                            setOpen(true);
                        }}
                        onSlideChange={async (swiper) => {
                            ...
                            if (
                                movies.length - swiper.activeIndex <= 20 &&
                                hasNextPage &&
                                !isFetching
                            ) {
                                await fetchNextPage();
